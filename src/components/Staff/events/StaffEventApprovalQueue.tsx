...
export default function StaffEventApprovalQueue() {
  ...
  const openDetailModal = async (eventId: string) => {
    ...
    if (isSameEvent && hasSpecForThisEvent) return;
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.message ?? err?.message ?? "Không thể tải chi tiết sự kiện"
      );
    }
  };
  ...
  const handleRejectFromModal = async () => {
    ...
    try {
      await dispatch(
        fetchRejectPublishEvent({ eventId: selectedEventId, reason: rejectReason }),
      ).unwrap();
          ...
          {currentCount > 0 && (
            <div className="px-8 py-4 border-t border-[#302447] flex flex-col md:flex-row items-start md:items-center justify-between gap-3 bg-white/5">
              <p className="text-xs text-[#a592c8]">
                ...
                trên <span className="text-white font-bold">{currentCount}</span> sự kiện
              </p>
                ...
                <div className="hidden sm:flex items-center gap-1">
                  {Array.from({ length: displayTotalPages }, (_, idx) => idx + 1).map((p) => {
                    ...
                    if (!show) {
                      ...
                      if (p === displayTotalPages - 1 && page < displayTotalPages - 5)
                        return <span key={p} className="px-2">...</span>;
                      return null;
                    }
                    return (
