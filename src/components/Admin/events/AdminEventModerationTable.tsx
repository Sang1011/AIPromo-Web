...
export default function AdminEventModerationTable() {
  ...
  const [selectedRevenueEventId, setSelectedRevenueEventId] = useState<
    string | null
  >(null);
  const revenueDetail = useSelector(
    (state: RootState) => state.ADMIN_EVENT_REPORT.revenueDetail,
  );
  const revenueCurrentEventId = useSelector(
    (state: RootState) => state.ADMIN_EVENT_REPORT.currentEventId,
  );
  const revenueLoading = useSelector(
    (state: RootState) => state.ADMIN_EVENT_REPORT.loading,
  );
  const revenueError = useSelector(
    (state: RootState) => state.ADMIN_EVENT_REPORT.error,
  );
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={5} className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-[#a592c8] text-sm">
              ...
              currentPageEvents.map((evt) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 min-w-0">
                    <span
                      className="text-sm text-[#a592c8] truncate block min-w-0"
                      title={evt.location}
                    >
                      {evt.location || "—"}
                    </span>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="inline-flex items-center gap-2 px-2.5 py-1.5 rounded-md bg-[#1a122b]/80 border border-[#302447] w-fit group-hover:border-emerald-500/30 transition-all">
                        ...
                        <div className="flex flex-col leading-tight">
                          <span className="text-[8px] text-[#524a6e] uppercase font-bold">
                            Bắt đầu
                          </span>
                          <div className="flex items-center gap-1.5">
                            <span className="text-xs font-semibold text-emerald-400">
                              {formatTime(evt.eventStartAt)}
                            </span>
                            <span className="text-[10px] text-[#a592c8]">
                              •
                            </span>
                            <span className="text-xs font-medium text-white">
                              {formatDate(evt.eventStartAt)}
                            </span>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="flex flex-col leading-tight">
                          ...
                          <div className="flex items-center gap-1.5">
                            <span className="text-xs font-semibold text-red-400">
                              {formatTime(evt.eventEndAt)}
                            </span>
                            <span className="text-[10px] text-[#a592c8]">
                              •
                            </span>
                            <span className="text-xs font-medium text-white">
                              {formatDate(evt.eventEndAt)}
                            </span>
                          </div>
                        </div>
                      </div>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                ...
                <div>
                  ...
                  {eventSpecLoading && eventSpecEventId === selectedEventId ? (
                    ...
                    eventSpecEventId === selectedEventId ? (
                      ...
                      {currentEvent.ticketTypes.map((t: any) => (
                        ...
                        >
                          ...
                          <span className="text-purple-400 font-bold">
                            {t.price?.toLocaleString?.() ?? t.price}₫
                          </span>
                        </div>
                      ))}
                    </div>
              ...
              <div className="p-6 border-t border-purple-500/30 flex justify-end gap-4 bg-[#18122B]">
                <button
                  ...
                  className="px-6 py-3 rounded-xl border border-white/10 text-slate-300"
                >
                  Đóng
                </button>
              </div>
          ...
          <p className="text-[10px] text-[#a592c8] font-bold uppercase tracking-widest">
            ...
            <span className="text-primary">
              {Math.min(
                ...
                pagination.totalCount ?? filteredEvents.length,
              )}
            </span>{" "}
            trên{" "}
              ...
              (_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`min-w-[36px] p-2 rounded-lg transition-all font-bold text-xs ${
