...
export default function LockSeatTab({ selectedSessionId }: LockSeatTabProps) {
    ...
    const gotoEdit = () => navigate(`${location.pathname}/edit`);

    // Fetch seat map whenever session changes
    useEffect(() => {
            ...
            <div className="flex gap-6 flex-1 min-w-0 overflow-hidden">
                ...
                ">
                    ...
                    <div className="flex-1 space-y-3 overflow-hidden">
                        <div className={`space-y-3 pr-1 ${showAll ? "max-h-[260px] overflow-y-auto" : ""}`}>
                            {selectedSeatIds.length === 0 && (
                                <div className="text-sm text-slate-500">Chưa chọn ghế nào</div>
                            )}
                            ...
                            {visibleSeats.map(id => {
                                ...
                                return (
                                    <div
                                        key={id}
                                        className="relative flex items-center gap-3 px-4 py-3 rounded-2xl border border-primary/30 bg-primary/10"
                                    >
                                        ...
                                        <button
                                            onClick={() => setSelectedSeatIds(prev => prev.filter(x => x !== id))}
                        ...
                        {hiddenCount > 0 && !showAll && (
                            <button
                                ...
                                className="mt-3 w-full py-3 rounded-xl border border-dashed border-white/10 text-sm text-slate-400 hover:text-white hover:border-white/20"
                            >
                    ...
                    <div className="mt-5 space-y-3">
                        ...
                        <button
                            onClick={() => { if (currentEvent) gotoEdit(); }}
                            disabled={currentEvent?.status !== "Draft"}
                            className="
                                w-full py-3 rounded-xl
                                bg-gradient-to-r from-primary/30 to-primary/10
                                text-white font-semibold border border-primary/30 transition-all
                                hover:from-primary/40 hover:to-primary/20 hover:border-primary/50
                                hover:shadow-[0_0_18px_2px_rgba(139,92,246,0.35)]
                                disabled:opacity-40 disabled:cursor-not-allowed
                                disabled:hover:shadow-none disabled:hover:from-primary/30 disabled:hover:to-primary/10
                            "
                        >
                            Chỉnh sửa sơ đồ
                        </button>
